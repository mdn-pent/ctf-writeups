...
We can use [[Nmap]] to scan oer target :
...
and if we go to /whatever , in the [[Source Code]] we can find a hidden text that looks like a [[Base64]] encoded string :

![61ee5d31b7054d4d20342653faf96d06.png](61ee5d31b7054d4d20342653faf96d06.png)
...
we can use again [[Gobuster]] to gain info about the other [[Server]] on port 65524 :
...
We can use [[HashIdentifier]] to [[Decipher]] :
...
and now if we go to the main page on port 65524 and in the [[Source Code]] we find flag3 in clear text:

<img src="../../Flameshots/06daf159a86267455a39ff70c83bf8c4.png" alt="06daf159a86267455a39ff70c83bf8c4.png" width="383" height="100">

on the same page we find what looks like another string that we need to [[Decode]]. The hint says: “its encoded with ba..”

![3109e8fb4fa41289d7ea1148473eeae0.png](3109e8fb4fa41289d7ea1148473eeae0.png)

if we decode it in [[Base62]] we notice it's a hidden directory : 

<img src="../../Flameshots/dcb1b33e4bf4d36f97453ad4d6204191.png" alt="dcb1b33e4bf4d36f97453ad4d6204191.png" width="528" height="332">

When we visit this directory, we are greeted with a page with a picture once again. we save the image and check it with [[Steghide]]. We are requested a password for this. The page source reveals another [[Hash]] that we will need to crack. We are provided with a wordlist and a hint on what format the hash is. Time to fire up [[JohnTheRipper]]:
...
we can use the cracked passwd to extract from the image with [[Steghide]] :
...
![f983327bce5868a109bc34dcd6cbb0c3.png](f983327bce5868a109bc34dcd6cbb0c3.png)

# First Access
## SSH

now with this passwd we can access [[SSH]] with the username : boring and the cracked passwd (on port 6498 see nmap_scan) :
...
when we read the file, it seems our [[Flag]] got a bit messed up! The hint says it got rotated. We can easily find an online [[ROT]] decrypter. It turns out its rot -13.

<span style="color: rgb(45, 194, 107);">**flag{n0wits33msn0rm4l}**</span>

# Privilege Escalation

Now we can do [[Privilege Escalation]], we check crontabs we see there's a cronjob we can replace with a [[Shell]] :
...
Time to [[3 - Tags/Hacking Concepts/Reverse Shell]] this! I start with setting up my [[Netcat]] listener and then I add a [[Bash]] [[3 - Tags/Hacking Concepts/Reverse Shell]] (from pentestmonkeys) to the cronjob file:
