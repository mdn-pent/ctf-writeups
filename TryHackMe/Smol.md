...
First thing we do is checking if password is allowed on access on [[SSH]] :
...
We search for hidden directories with [[Dirsearch]] :
...
Looks like we have a [[Wordpress]] [[Server]], we have to update also the /etc/hosts to catch www.smol.thm :

![[Pasted image 20250720213518.png]]

There's username [[Enumeration]] .
### WPScan 

We can use [[WPScan]] which is a [[Wordpress]] scanner for vulnerabilities, we can use it without a API token or make a free account on their website to get an API for better results :
...
If we search for JSmol2WP SSRF we find info about [[CVE-2018-20463]] :

![[Pasted image 20250720215506.png]]
...
![[Pasted image 20250721131646.png]]
...
![[Pasted image 20250721132055.png]]

Boom. Now we find something :

![[Pasted image 20250721132244.png]]

Let's search about 'Holly Dolly' plugin :

![[Pasted image 20250721132629.png]]

So we know the name of the file .

We can try [[Escaping]] to the file :

![[Pasted image 20250721133317.png]]

And we have the same code, but we can spot the difference from original project in the eval function in [[Base64]], so we decode it :
...
![[Pasted image 20250721134114.png]]
...
Now we create the file and open a basic [[Python]] [[Server]] :
...
![[Pasted image 20250721134847.png]]

We make it executable with `chmod 777 sell.sh` and then open our listener with [[Netcat]] and execute :

![[Pasted image 20250721135215.png]]

And we have it .

### Shell Stabilization

Now we can do [[Shell Stabilization]] :
...
We can use [[MySQL]] to try to access it and read something :
...
We have users and hashes so we create a list and try to crack them for [[Lateral Movement]] :
...
We can use [[Hashcat]] to find the passwords :
...
We will use [[JohnTheRipper]] to gain speed :
...
So we have a private key for [[SSH]] . Let's copy it in a file :
...
We can use the key to access [[SSH]] as `think` user after `chmod 600 key` :
...
We can bypass this simply hosting a [[Python]] [[Server]] and download it to our machine :
...
Now we can use [[Zip2John]] to create the hash and then crack the hash for the zip file with [[JohnTheRipper]] :
...
We copy the hash in a file and crack with [[JohnTheRipper]] :
```
┌──(hax㉿HaxonKali)-[~/THM/CHALLENGES/Easy/Smol]
└─$ john hashes.txt --wordlist=/usr/share/wordlists/rockyou.txt  
