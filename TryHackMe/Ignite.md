...
So we have just a [[Website]] .
# Enumeration

## HTTP (80)

We can start [[Enumeration]] on the [[Webpage]] on port 80 :

![[Pasted image 20250719210419.png]]

So we go to /fuel and try these credentials :

![[Pasted image 20250719210605.png]]
...
We search for Fuel CMS with [[Searchsploit]] :
...
Now we can obtain a [[Shell]], first we open a listener with [[Netcat]] :
...
And we have the shell in our listener but it's not stable so we'll going through a [[Shell Stabilization]] .
...
And continue with the [[Privilege Escalation]] to find the root flag, first thing we check for permissions with `sudo -l` but we don't have access without password .
