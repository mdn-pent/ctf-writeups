...
![[Pasted image 20250721153607.png]]
...
We set up our listener with [[Netcat]] :
...
now we paste the [[Reverse Shell]] in the python server :
...
We can now perform a [[Shell Stabilization]] :
...
And a password . We have a [[Github]] account, let's try password reusing on [[SSH]] :
...
We have it but we are www-data, but if we modify the endpoint to the admin user, maybe we can have a root [[Shell]] here. 
## Custom Scripting (Python)

We can create a custom script in [[Python]] :
