...
We add the host in /etc/hosts (sudo.thm) and scan with our [[Nmap]] tool :
...
We start [[Enumeration]] on the http [[Server]] :
...
We run [[FFuf]] and check **Size** then re-run with correct filter-size (-fs) :
...
We modify the user-agent in the request in [[Developer tools]] and resend it, we find a page:
...
We don't have anonymous access in [[FTP]] .

### Hydra

We can crack the password of the found user with [[Hydra]] using her wizard:
...
Now we can enter [[FTP]] to see what's there and get it to our directory :
...
We use [[Stegcracker]] to crack the password for extraction :
...
We don't have anonymous access in [[SSH]] .
...
we check for sudo -l for the [[Privilege Escalation]] :
...
So we search for  `(All, !root) /bin/bash` and we got the [[Exploit]] :
